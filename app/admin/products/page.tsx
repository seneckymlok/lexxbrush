...
export default function AdminProductsPage() {
  ...
  if (loading) {
    return <div className="flex items-center justify-center h-64"><div className="text-white/30 text-sm">Loading products...</div></div>;
  }
  ...
  return (
    <div>
      <div className="flex items-center justify-between mb-8">
        ...
        <Link href="/admin/products/new" className="px-4 py-2 bg-white text-black text-sm font-medium rounded-lg hover:bg-white/90 transition-colors">+ Add Product</Link>
      </div>
      ...
      {products.length === 0 ? (
        <div className="bg-white/[0.02] border border-white/5 rounded-xl px-5 py-16 text-center">
          ...
          <Link href="/admin/products/new" className="text-white/60 hover:text-white text-sm underline">Add your first product</Link>
        </div>
      ) : (
        <div className="grid gap-3">
          {products.map((product) => (
            <div key={product.id} className="bg-white/[0.02] border border-white/5 rounded-xl px-5 py-4 flex items-center gap-4 hover:bg-white/[0.03] transition-colors">
              ...
              <Link href={`/admin/products/${product.id}/edit`} className="text-xs text-white/30 hover:text-white/60 transition-colors">Edit</Link>
              <button onClick={() => deleteProduct(product.id)} className="text-xs text-red-400/40 hover:text-red-400 transition-colors">Delete</button>
            </div>
          ))}
        </div>
      )}
