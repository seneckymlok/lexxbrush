export type Locale = "en" | "sk";

export const translations = {
  en: {
    nav: {
      shop: "Shop",
      custom: "Custom Order",
      cart: "Cart",
      contact: "Contact",
    },
    hero: {
      tagline: "Hand-Airbrushed Wearable Art",
      subtitle: "Every piece is unique. Painted entirely by hand.",
      cta: "Shop Collection",
    },
    shop: {
      title: "Collection",
      filter: "Filter",
      all: "All",
      oneOfAKind: "One of One",
      addToCart: "Add to Cart",
      selectSize: "Select Size",
      sold: "Sold",
      viewDetails: "View Details",
    },
    product: {
      oneOfAKind: "One of One",
      size: "Size",
      addToCart: "Add to Cart",
      sold: "Sold Out",
      backToShop: "Back to Shop",
      description: "Description",
      details: "Details",
      handPainted: "Hand-airbrushed",
      unique: "Unique piece",
      madeByHand: "Made by hand",
      zoomHintTouch: "Pinch to zoom",
      zoomHintMouse: "Hold to zoom",
    },
    custom: {
      title: "Custom Order",
      subtitle: "Want something truly yours? Describe your vision and I'll bring it to life with airbrush.",
      name: "Your Name",
      namePlaceholder: "",
      email: "Your Email",
      emailPlaceholder: "",
      description: "Describe Your Vision",
      descriptionPlaceholder: "Tell me what you want - colors, style, garment type, references...",
      garment: "Garment Type",
      garmentPlaceholder: "e.g. Hoodie, T-shirt, Backpack...",
      budget: "Budget Range",
      budgetPlaceholder: "e.g. 100 - 200 €",
      submit: "Send Request",
      success: "Request sent! I'll get back to you soon.",
      error: "Something went wrong. Please try again.",
    },
    cart: {
      title: "Cart",
      empty: "Your cart is empty",
      continueShopping: "Continue Shopping",
      remove: "Remove",
      total: "Total",
      checkout: "Checkout",
      size: "Size",
    },
    checkout: {
      title: "Checkout",
      contact: "Contact",
      shipping: "Delivery",
      email: "Email",
      emailPlaceholder: "",
      name: "Full Name",
      namePlaceholder: "",
      phone: "Phone (optional)",
      phonePlaceholder: "e.g. +421 900 123 456",
      country: "Country",
      deliveryMethod: "Delivery Method",
      pickupPoint: "Pickup Point",
      homeDelivery: "Home Delivery",
      choosePickupPoint: "Choose Pickup Point",
      enterDeliveryAddress: "Enter Delivery Address",
      changeSelection: "Change",
      selectedPoint: "Selected Pickup Point",
      selectedAddress: "Delivery Address",
      noDeliverySelected: "Please select a delivery method",
      orderSummary: "Order Summary",
      placeOrder: "Place Order",
      pay: "Continue to Payment",
      processing: "Processing...",
      backToCart: "Back to Cart",
      successTitle: "Order Confirmed!",
      successMessage: "Thanks for your order. You'll receive a confirmation email shortly. Each piece is hand-airbrushed, so your unique artwork will be on its way soon.",
      free: "FREE",
    },
    contact: {
      title: "Contact",
      subtitle: "Have a question or want to collaborate? Get in touch.",
      name: "Your Name",
      namePlaceholder: "",
      email: "Your Email",
      emailPlaceholder: "",
      subject: "Subject",
      subjectPlaceholder: "e.g. Custom design inquiry",
      message: "Message",
      messagePlaceholder: "Write your message here...",
      send: "Send Message",
      success: "Message sent! I'll get back to you soon.",
      error: "Something went wrong. Please try again.",
      companyInfo: "Company Info",
      followUs: "Follow Me",
    },
    auth: {
      login: "Login",
      register: "Create Account",
      email: "Email",
      password: "Password",
      signIn: "Sign In",
      signingIn: "Signing in...",
      signUp: "Register",
      signingUp: "Creating account...",
      signOut: "Sign Out",
      fillAllFields: "Please fill out all required fields correctly.",
      passwordTooShort: "Password must be at least 6 characters.",
      passwordHint: "Minimum 6 characters",
      checkInbox: "Check Your Inbox",
      confirmationSent: "We sent a confirmation email to",
      resendEmail: "Resend confirmation email",
      resendIn: "Resend available in",
      emailResent: "Email sent successfully!",
      emailNotConfirmed: "Email not yet confirmed",
      checkSpam: "Please check your spam folder or request a new confirmation email.",
      backToLogin: "Back to Login",
      confirmed: {
        eyebrow: "Account · Verified",
        title: "You're in.",
        body: "Your email is confirmed and your Lexxbrush account is live. Sign in to track your orders, save your favorites, and get first access to every drop.",
        signIn: "Sign in",
        toAccount: "Go to my account",
        browse: "Browse the collection",
        errorEyebrow: "Link · Expired",
        errorTitle: "This link's no longer valid.",
        errorBody: "Confirmation links expire after 24 hours and work only once. No problem, request a fresh one and we'll send it straight over.",
        errorCta: "Get a new link",
      },
      forgot: {
        link: "Forgot password?",
        title: "Reset password",
        body: "Enter your email and we'll send you a link to set a new one.",
        cta: "Send reset link",
        sending: "Sending...",
        sentTo: "If an account exists for this address, a reset link is on its way to",
        resend: "Resend reset link",
      },
      reset: {
        title: "New password",
        body: "Choose a new password for your account. Make it count.",
        newPassword: "New password",
        confirmPassword: "Confirm password",
        cta: "Update password",
        saving: "Updating...",
        mismatch: "Passwords don't match.",
        expiredTitle: "This link's no longer valid.",
        expiredBody: "Password reset links expire after a short window and work only once. Request a fresh one and we'll send it straight over.",
        doneEyebrow: "Password · Updated",
        doneTitle: "You're all set.",
        doneBody: "Your password is updated and you're signed in. Taking you to your account.",
      },
      welcomeBack: "Welcome back to Lexxbrush",
      joinUs: "Join Lexxbrush to track your orders",
      noAccount: "Do not have an account?",
      yesAccount: "Already have an account?",
      account: "My Account",
      orderHistory: "Order History",
      myFavorites: "My Favorites",
      noFavorites: "You haven't saved any items yet.",
      noOrders: "You haven't placed any orders yet.",
      startShopping: "Start Shopping",
      status: "Status",
      total: "Total",
    },
    footer: {
      followUs: "Follow Us",
      instagram: "Instagram",
      madeBy: "Made by",
      rights: "All rights reserved.",
      terms: "Terms & Conditions",
      privacy: "Privacy Policy",
      shipping: "Shipping & Returns",
    },
    newsletter: {
      form: {
        ariaLabel:     "Newsletter signup",
        label:         "Join the drop list",
        placeholder:   "your@email.com",
        helper:        "Only when a new piece drops. Roughly once a month.",
        submit:        "Join",
        submitting:    "Joining…",
        successInline: "Check your inbox",
        successDetail: "We just sent you a confirmation link. Tap it to finish.",
        invalid:       "That doesn't look like a valid email.",
        serverError:   "Something went wrong on our side. Try again in a moment.",
        networkError:  "Connection error. Please try again.",
      },
      checkout: {
        label: "Email me when new pieces drop. Roughly once a month - never more.",
      },
      confirmed: {
        eyebrow: "♠   You're in",
        title:   "Welcome to the drop.",
        body:    "You'll only hear from us when something new lands. No filler, no daily noise - just first looks.",
        cta:     "Browse the collection",
      },
      unsubscribed: {
        eyebrow: "♣   Removed",
        title:   "You're off the list.",
        body:    "You won't hear from us again. If this was a mistake, you can rejoin from the footer of any page.",
        cta:     "Back to the site",
      },
      invalid: {
        eyebrow: "Error",
        title:   "This link is no longer valid.",
        body:    "It may have already been used or expired. If you meant to subscribe or unsubscribe, you can do it from the footer of any page.",
        cta:     "Back to the site",
      },
    },
    notFound: {
      titleLine1: "WRONG",
      titleLine2: "CARD.",
      subtitle: "Pick one and try again",
      back: "Back to shop",
    },
    errorPage: {
      title: "OUT OF INK.",
      subtitle: "Something went wrong on our side. Try again or head back to the collection.",
      retry: "Try again",
      home: "Back to shop",
    },
  },
  sk: {
    nav: {
      shop: "Obchod",
      custom: "Na Zákazku",
      cart: "Košík",
      contact: "Kontakt",
    },
    hero: {
      tagline: "Ručne Airbrushované Nositeľné Umenie",
      subtitle: "Každý kus je unikát. Maľované úplne ručne.",
      cta: "Pozrieť Kolekciu",
    },
    shop: {
      title: "Kolekcia",
      filter: "Filter",
      all: "Všetko",
      oneOfAKind: "One of One",
      addToCart: "Pridať do Košíka",
      selectSize: "Vybrať Veľkosť",
      sold: "Predané",
      viewDetails: "Zobraziť Detail",
    },
    product: {
      oneOfAKind: "One of One",
      size: "Veľkosť",
      addToCart: "Pridať do Košíka",
      sold: "Vypredané",
      backToShop: "Späť do Obchodu",
      description: "Popis",
      details: "Detaily",
      handPainted: "Ručne airbrushované",
      unique: "Unikátny kus",
      madeByHand: "Vyrobené ručne",
      zoomHintTouch: "Priblíž štipnutím",
      zoomHintMouse: "Podrž pre priblíženie",
    },
    custom: {
      title: "Na Zákazku",
      subtitle: "Chceš niečo naozaj tvoje? Opíš svoju víziu a ja ju airbrushom oživím.",
      name: "Tvoje Meno",
      namePlaceholder: "",
      email: "Tvoj Email",
      emailPlaceholder: "",
      description: "Opíš Svoju Víziu",
      descriptionPlaceholder: "Povedz mi, čo chceš - farby, štýl, typ oblečenia, referencie...",
      garment: "Typ Oblečenia",
      garmentPlaceholder: "napr. Mikina, Tričko, Batoh...",
      budget: "Cenový Rozsah",
      budgetPlaceholder: "napr. 100 - 200 €",
      submit: "Odoslať Požiadavku",
      success: "Požiadavka odoslaná! Ozvem sa čoskoro.",
      error: "Niečo sa pokazilo. Skús to znova.",
    },
    cart: {
      title: "Košík",
      empty: "Tvoj košík je prázdny",
      continueShopping: "Pokračovať v Nákupe",
      remove: "Odstrániť",
      total: "Celkom",
      checkout: "Objednať",
      size: "Veľkosť",
    },
    checkout: {
      title: "Objednávka",
      contact: "Kontakt",
      shipping: "Doručenie",
      email: "Email",
      emailPlaceholder: "",
      name: "Celé Meno",
      namePlaceholder: "",
      phone: "Telefón (voliteľné)",
      phonePlaceholder: "napr. +421 900 123 456",
      country: "Krajina",
      deliveryMethod: "Spôsob Doručenia",
      pickupPoint: "Odberné Miesto",
      homeDelivery: "Doručenie na Adresu",
      choosePickupPoint: "Vybrať Odberné Miesto",
      enterDeliveryAddress: "Zadať Adresu Doručenia",
      changeSelection: "Zmeniť",
      selectedPoint: "Vybrané Odberné Miesto",
      selectedAddress: "Adresa Doručenia",
      noDeliverySelected: "Prosím, vyberte spôsob doručenia",
      orderSummary: "Súhrn Objednávky",
      placeOrder: "Objednať",
      pay: "Pokračovať k Platbe",
      processing: "Spracováva sa...",
      backToCart: "Späť do Košíka",
      successTitle: "Objednávka potvrdená!",
      successMessage: "Ďakujeme za objednávku. Čoskoro dostanete potvrdzovací email. Každý kus je ručne maľovaný, takže vaše unikátne umenie bude čoskoro na ceste.",
      free: "ZADARMO",
    },
    contact: {
      title: "Kontakt",
      subtitle: "Máš otázku alebo chceš spolupracovať? Napíš mi.",
      name: "Tvoje Meno",
      namePlaceholder: "",
      email: "Tvoj Email",
      emailPlaceholder: "",
      subject: "Predmet",
      subjectPlaceholder: "napr. Zákazka na mieru",
      message: "Správa",
      messagePlaceholder: "Napíš svoju správu sem...",
      send: "Odoslať Správu",
      success: "Správa odoslaná! Ozvem sa čoskoro.",
      error: "Niečo sa pokazilo. Skús to znova.",
      companyInfo: "Informácie o firme",
      followUs: "Sleduj Ma",
    },
    auth: {
      login: "Prihlásenie",
      register: "Vytvoriť Účet",
      email: "Email",
      password: "Heslo",
      signIn: "Prihlásiť",
      signingIn: "Prihlasujem...",
      signUp: "Registrovať",
      signingUp: "Vytváram účet...",
      signOut: "Odhlásiť",
      fillAllFields: "Prosím, vyplňte všetky povinné polia správne.",
      passwordTooShort: "Heslo musí mať aspoň 6 znakov.",
      passwordHint: "Minimálne 6 znakov",
      checkInbox: "Skontroluj Email",
      confirmationSent: "Poslali sme potvrdzovací email na",
      resendEmail: "Poslať znova",
      resendIn: "Možnosť opätovného zaslania za",
      emailResent: "Email úspešne odoslaný!",
      emailNotConfirmed: "Email ešte nebol potvrdený",
      checkSpam: "Skontroluj spam priečinok alebo si vyžiadaj nový potvrdzovací email.",
      backToLogin: "Späť na prihlásenie",
      confirmed: {
        eyebrow: "Účet · Overený",
        title: "Si dnu.",
        body: "Tvoj email je potvrdený a tvoj Lexxbrush účet je aktívny. Prihlás sa, sleduj svoje objednávky, ukladaj si obľúbené kúsky a maj prístup ku každému dropu medzi prvými.",
        signIn: "Prihlásiť sa",
        toAccount: "Prejsť na môj účet",
        browse: "Prezrieť kolekciu",
        errorEyebrow: "Odkaz · Vypršal",
        errorTitle: "Tento odkaz už neplatí.",
        errorBody: "Potvrdzovacie odkazy vypršia po 24 hodinách a fungujú len raz. Žiaden problém, vyžiadaj si nový a hneď ti ho pošleme.",
        errorCta: "Získať nový odkaz",
      },
      forgot: {
        link: "Zabudol si heslo?",
        title: "Obnova hesla",
        body: "Zadaj svoj email a pošleme ti odkaz na nastavenie nového hesla.",
        cta: "Poslať odkaz",
        sending: "Odosielam...",
        sentTo: "Ak pre túto adresu existuje účet, odkaz na obnovu je na ceste na",
        resend: "Poslať odkaz znova",
      },
      reset: {
        title: "Nové heslo",
        body: "Zvoľ si nové heslo pre svoj účet.",
        newPassword: "Nové heslo",
        confirmPassword: "Potvrď heslo",
        cta: "Aktualizovať heslo",
        saving: "Aktualizujem...",
        mismatch: "Heslá sa nezhodujú.",
        expiredTitle: "Tento odkaz už neplatí.",
        expiredBody: "Odkazy na obnovu hesla vypršia po krátkom čase a fungujú len raz. Vyžiadaj si nový a hneď ti ho pošleme.",
        doneEyebrow: "Heslo · Aktualizované",
        doneTitle: "Hotovo.",
        doneBody: "Tvoje heslo je aktualizované a si prihlásený. Presmerúvam ťa na účet.",
      },
      welcomeBack: "Vitaj späť v Lexxbrush",
      joinUs: "Pridaj sa a sleduj svoje objednávky",
      noAccount: "Nemáš ešte účet?",
      yesAccount: "Už máš účet?",
      account: "Môj Účet",
      orderHistory: "História Objednávok",
      myFavorites: "Obľúbené",
      noFavorites: "Zatiaľ tu nemáš žiadne obľúbené kúsky.",
      noOrders: "Zatiaľ nemáš žiadne objednávky.",
      startShopping: "Začať Nakupovať",
      status: "Stav",
      total: "Celkom",
    },
    footer: {
      followUs: "Sleduj Nás",
      instagram: "Instagram",
      madeBy: "Made by",
      rights: "Všetky práva vyhradené.",
      terms: "Obchodné Podmienky",
      privacy: "Ochrana Osobných Údajov",
      shipping: "Doprava a Vrátenie",
    },
    newsletter: {
      form: {
        ariaLabel:     "Prihlásenie na newsletter",
        label:         "Pridaj sa na zoznam dropov",
        placeholder:   "tvoj@email.com",
        helper:        "Ozveme sa len keď príde niečo nové. Zvyčajne raz za mesiac.",
        submit:        "Pridať sa",
        submitting:    "Pridávam…",
        successInline: "Skontroluj e-mail",
        successDetail: "Práve sme ti poslali potvrdzovací link. Klikni naň a si v zozname.",
        invalid:       "Toto nevyzerá ako platná e-mailová adresa.",
        serverError:   "Niečo sa pokazilo na našej strane. Skús to znova o chvíľu.",
        networkError:  "Chyba spojenia. Skús to znova.",
      },
      checkout: {
        label: "Daj mi vedieť e-mailom keď príde nový drop. Zvyčajne raz za mesiac - nikdy viac.",
      },
      confirmed: {
        eyebrow: "♠   Si na zozname",
        title:   "Vitaj v drope.",
        body:    "Ozveme sa len keď príde niečo nové. Žiadny denný šum - len prvé pozretie.",
        cta:     "Pozrieť kolekciu",
      },
      unsubscribed: {
        eyebrow: "♣   Odhlásený/á",
        title:   "Si mimo zoznamu.",
        body:    "Už ti nepošleme žiadne e-maily. Ak to bolo omylom, pridať sa môžeš z päty každej stránky.",
        cta:     "Späť na stránku",
      },
      invalid: {
        eyebrow: "Chyba",
        title:   "Tento link už nie je platný.",
        body:    "Možno už bol použitý alebo vypršala jeho platnosť. Prihlásiť alebo odhlásiť sa môžeš z päty každej stránky.",
        cta:     "Späť na stránku",
      },
    },
    notFound: {
      titleLine1: "ZLÁ",
      titleLine2: "KARTA.",
      subtitle: "Vyber si jednu a skús to znova",
      back: "Späť do obchodu",
    },
    errorPage: {
      title: "DOŠLA FARBA.",
      subtitle: "Niečo sa pokazilo na našej strane. Skús to znova alebo sa vráť do kolekcie.",
      retry: "Skús to znova",
      home: "Späť do obchodu",
    },
  },
} as const;

export function t(locale: Locale, path: string): string {
  const keys = path.split(".");
  let result: unknown = translations[locale];
  for (const key of keys) {
    if (result && typeof result === "object" && key in result) {
      result = (result as Record<string, unknown>)[key];
    } else {
      return path;
    }
  }
  return result as string;
}
